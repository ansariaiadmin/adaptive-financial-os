...
echo -e "${CYAN}"
cat <<'BANNER'
Adaptive Financial OS — v3.1.0 — تاریکی روشن شد — پشتیبانی صفر
مالی + دفترداری دوبل + Kafka + GraphQL + ناتیف — تاریکی روشن شد
Email + SMS مالی + Telegram حسابدار
BANNER
echo -e "${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  🧙‍♂️ جادوگر نصب Adaptive Financial OS v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}  Email + SMS مالی + Telegram حسابدار${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}سلام! 👋 مالی + دفترداری دوبل + Kafka + GraphQL + ناتیف — تاریکی روشن شد${NC}"
...
if [ "adaptive-financial-os" = "aark-kernel" ]; then
...
if [ "adaptive-financial-os" = "legal-platform" ]; then
...
if [ "adaptive-financial-os" = "universal-document-os" ]; then
...
if [ "adaptive-financial-os" = "eaos" ]; then
...
if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && command -v curl &> /dev/null; then
    ...
    if [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🧪 تست Adaptive Financial OS v3.1.0 — تاریکی روشن شد — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1 && ok "Telegram پیام تست فرستاده شد — چک کن — تاریکی روشن شد" || err "Telegram پیام fail — Chat ID چک کن — ربات استارت کردی؟ — تاریکی روشن شد"
    fi
  fi
...
if [ "$SKIP_ENV" = "true" ]; then
  ...
  cat > .env <<EOF
# Adaptive Financial OS — .env — v3.1.0 — تاریکی روشن شد — $(date)
...
# Outbox Relay — Kafka — تاریکی روشن شد
...
ADMIN_EMAIL=admin@adaptive-financial-os.dev
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  🎉 جادو تمام! Adaptive Financial OS آماده — تاریکی روشن شد! 🎉${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${BOLD}${BLUE}📍 دسترسی:${NC}"
echo -e "${GREEN}  🌐 http://localhost:3000 — مالی + دفترداری دوبل + Kafka + GraphQL + ناتیف — تاریکی روشن شد${NC}"
