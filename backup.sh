...
BACKUP_FILE="$BACKUP_DIR/adaptive-financial-os-backup-$TIMESTAMP.tar.gz"
BACKUP_ENCRYPTED="$BACKUP_DIR/adaptive-financial-os-backup-$TIMESTAMP.tar.gz.enc"
...
if command -v docker &> /dev/null && docker compose ps 2>/dev/null | grep -q "Up"; then
  ...
  docker compose exec -T db pg_dump -U adaptive-financial-os adaptive-financial-os 2>/dev/null > /tmp/adaptive-financial-os-db-$TIMESTAMP.sql || echo "DB dump fail — شاید DB خاموشه"
  ok "DB dump — /tmp/adaptive-financial-os-db-$TIMESTAMP.sql"
else
...
if [ -f /tmp/adaptive-financial-os-db-$TIMESTAMP.sql ]; then
  tar -rf $BACKUP_FILE /tmp/adaptive-financial-os-db-$TIMESTAMP.sql 2>/dev/null || true
...
ls -t $BACKUP_DIR/adaptive-financial-os-backup-*.tar.gz* 2>/dev/null | tail -n +8 | xargs rm -f 2>/dev/null || true
