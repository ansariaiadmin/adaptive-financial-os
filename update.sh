...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Adaptive Financial OS — Double-Entry Ledger - آپدیت خودکار${NC}"
...
if command -v curl &> /dev/null; then
  if curl -sf http://localhost:3000/api/health >/dev/null 2>&1 || curl -sf http://localhost:3000 >/dev/null 2>&1 || curl -sf http://localhost:8000 >/dev/null 2>&1 || curl -sf http://localhost:8080 >/dev/null 2>&1; then
    ...
    curl -sf http://localhost:3000/api/health 2>&1 | head -n 5 || true
  else
...
echo -e "${GREEN}========================================${NC}"
echo "آدرس / URL: http://localhost:3000"
