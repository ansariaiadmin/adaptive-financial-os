# 🧙‍♂️ جادوگر نصب Adaptive Financial OS — Double-Entry Ledger — فوق ساده!
...
## 🎯 Adaptive Financial OS — Double-Entry Ledger چیه؟ به زبان ساده:

سیستم مالی دفترداری دوبل + Event Sourcing + Kafka + GraphQL

**ویژگی‌های سقف 10/10:**
Double-entry balanced via trigger + immutability + RLS + idempotency + outbox + audit + Kafka idempotent + GraphQL Query/Mutation/Subscription + Event Replay + Snapshot + FX + Web UI — سقف 10/10 — Financial-Grade
...
....................
✓ آماده! — http://localhost:3000 (Web UI) + http://localhost:8000/docs (API)
```

### قدم ۳: استفاده (۱۰ ثانیه)

مرورگر → `http://localhost:3000 (Web UI) + http://localhost:8000/docs (API)`

**چی کار کن؟**
1. Web UI localhost:3000 → Journal Entries + Outbox+Kafka + Replay+Snapshot+FX 2. GraphQL Playground 3. POST /ledger/entries با idempotency
...
**برای غیر فنی:** فقط `install.sh` → مرورگر → http://localhost:3000 (Web UI) + http://localhost:8000/docs (API) — همین! 🎉
